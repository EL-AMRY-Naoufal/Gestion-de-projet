...
@Component({
  ...
  standalone: true,
  imports: [MatDialogModule, UserCardComponent, NgSwitch, NgSwitchCase, NgIf,CommonModule,FormsModule,MenuComponent],
...
export class ListUsersComponent {
    ...
    this._usersService.searchUsers(this.searchQuery.trim()).subscribe({
      next: (data: User[]) => {
        if (Array.isArray(data)) {
          ...
          console.error('La réponse de la recherche n\'est pas un tableau:', data);
          this._listUsers = [];
        }
      },
      error: (err) => {
  ...
  filterByRole() {
    if (this.selectedRole) {
      this._usersService.searchUsersByRole(this.selectedRole).subscribe(data => {
        this._listUsers = data;
        })
    } else {
      this.listUsers;  
